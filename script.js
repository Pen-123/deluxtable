...
function updateBreakAndSchoolTimers(lessons, nowSeconds) {
  ...
  if (activeBreak) { breakLabel.textContent = "Break Ends"; breakTimer.textContent = formatCountdown(getTimeInSeconds(activeBreak.end) - nowSeconds); }
  else if (nextBreak) { breakLabel.textContent = "Next Break"; breakTimer.textContent = formatCountdown(getTimeInSeconds(nextBreak.start) - nowSeconds); }
  else { breakLabel.textContent = "Breaks Done"; breakTimer.textContent = "00:00:00"; }
...
async function importImageFile(file) {
  ...
  status.textContent = "Reading timetable image…"; progress.style.width = "8%";
  try {
    await ensureTesseract(); progress.style.width = "20%";
    ...
    progress.style.width = "0%"; showToast("Image import needs a cleaner timetable image.", true);
...
function buildKnownGridFromOCR(cells) {
  ...
  for (const day of Object.keys(expected)) {
    ...
    for (const subject of expected[day]) { totalPossible++; const target = subject.toLowerCase().replace(/[^a-z]/g, ""); if (found.some(text => text.includes(target) || target.includes(text))) totalScore++; }
  }
...
function parseSubjectLine(text) {
  ...
  if (/BREAK/.test(upper)) return { subject: "Break" }; if (/LUNCH/.test(upper)) return { subject: "Lunch" };
  ...
  if (/MSC\s*\/?\s*MED/.test(upper)) return { subject: "MSC / MED" }; if (/UAE\s*SS/.test(upper)) return { subject: "UAE Social Studies" };
  for (const token of upper.split(/\s+/)) if (SUBJECT_ALIASES[token]) return { subject: SUBJECT_ALIASES[token] };
  return null;
}
function isLikelySubjectLine(text) { return /ASSEMBLY|BREAK|LUNCH|MSC|MED|UAE|PE|BOYS|GIRLS|SWIN|FRE|ARA|ISL|ENG|MAT|SCI|IT|ART|MUS|HUM|FRENCH|ARABIC|ENGLISH|MATH|SCIENCE|MUSIC|HUMANITIES/i.test(text); }
...
setupControls(); updateClock(); setInterval(updateClock, 1000);
